...
The Jostle (`JSL`) provider registers **321** services across **14** JCA service types. Each list is the set of algorithm names (and OID aliases) registered for that type.
...
43. `RSA`
44. `RSA-KTS-KEM-KWS`
45. `RSA/ECB/PKCS1PADDING`
46. `SM4`
47. `SM4/CCM/NOPADDING`
...
The Jostle FIPS (`JSLFIPS`) provider registers **186** services against a 3.1.2 module (**262** against a 3.5.x one) across **13** JCA service types — what the OpenSSL FIPS module serves, not a subset filtered against its security policy. The module decides what is available: its implementations carry a `fips=yes`/`fips=no` property and the lib ctx's `fips=yes` default query excludes the latter, so ChaCha20 and OCB (for instance) are simply not fetchable — and on the 3.1.2 module Triple-DES too, though the 3.5.x module does serve it.
