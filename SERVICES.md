...
The Jostle (`JSL`) provider registers **299** services across **14** JCA service types. Each list is the set of algorithm names (and OID aliases) registered for that type.
...
The Jostle FIPS (`JSLFIPS`) provider registers **164** services against a 3.1.2 module (**237** against a 3.5.x one) across **13** JCA service types — what the OpenSSL FIPS module serves, not a subset filtered against its security policy. The module decides what is available: its implementations carry a `fips=yes`/`fips=no` property and the lib ctx's `fips=yes` default query excludes the latter, so Triple-DES, ChaCha20 and OCB (for instance) are simply not fetchable.
