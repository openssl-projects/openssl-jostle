 ...
 * Caller (Java) owns the result and frees it with JoFFM_freeUnsecureNullSafe.
 *
 * Deliberately implemented here rather than re-including the base tree's
 * openssl_ffm.c twin, which is how the FIPS library used to obtain it. That
 ...
 * only openssl_fips_jni.c, with its own getOSSLErrors); this brings FFM into
