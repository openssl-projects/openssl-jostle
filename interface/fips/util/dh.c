...
/*
 * Did set_peer fail because the peer's public value failed OpenSSL's own
 * DH public-key check (not in the subgroup generated by q, or otherwise
 * structurally invalid)? Called only after a set_peer failure that
 * peer_encoding_form_differs has already ruled out, same discipline.
 *
 * DH_R_CHECK_PUBKEY_INVALID, DH_R_INVALID_PUBKEY, DH_R_CHECK_PUBKEY_TOO_SMALL
 * and DH_R_CHECK_PUBKEY_TOO_LARGE all mean the same thing to a caller: the
 * public value failed the check. One code serves all four, since the remedy
 * does not differ.
 */
static int peer_pubkey_check_failed(void) {
    unsigned long err = ERR_peek_last_error();
    if (ERR_GET_LIB(err) != ERR_LIB_DH) {
        return 0;
    }
    int reason = ERR_GET_REASON(err);
    return reason == DH_R_CHECK_PUBKEY_INVALID
        || reason == DH_R_INVALID_PUBKEY
        || reason == DH_R_CHECK_PUBKEY_TOO_SMALL
        || reason == DH_R_CHECK_PUBKEY_TOO_LARGE;
}
...
int32_t dh_make_public_from_components(key_spec *spec,
    ...
    int32_t ret_code = dh_fromdata(spec, p_be, p_len, q_be, q_len, g_be, g_len,
                                   y_be, y_len, NULL, 0);
    if (ret_code != JO_SUCCESS || q_be == NULL) {
        return ret_code;
    }

    EVP_PKEY_CTX *check_ctx = EVP_PKEY_CTX_new_from_pkey(
            get_global_jostle_fips_ossl_lib_ctx(), spec->key, NULL);
    if (check_ctx == NULL) {
        EVP_PKEY_free(spec->key);
        spec->key = NULL;
...
int32_t dh_kex_set_peer(dh_kex_ctx *ctx, const key_spec *peer_pub,
    ...
    if (OPS_OPENSSL_ERROR_1 1 != EVP_PKEY_derive_set_peer(
        ...
        if (!JO_ERROR_WAS_INJECTED(generic)) {
            if (peer_encoding_form_differs(ctx->pctx, peer_pub->key)) {
                return JO_DH_PEER_ENCODING_MISMATCH;
            }
            if (peer_pubkey_check_failed()) {
                return JO_DH_PEER_PUBKEY_INVALID;
            }
        }
        return generic;
