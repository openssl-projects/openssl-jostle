...
static EC_GROUP *curve_group_from_name(const char *curve_name, int *nid_out) {
    ...
    EC_GROUP *group = EC_GROUP_new_by_curve_name_ex(
            get_global_jostle_fips_ossl_lib_ctx(), NULL, nid);
    if (group != NULL) {
        *nid_out = nid;
    }
    return group;
...
int32_t ec_get_curve_component(const char *curve_name, int32_t component,
    ...
    ctx = BN_CTX_new_ex(get_global_jostle_fips_ossl_lib_ctx());
    p = BN_new();
    a = BN_new();
    b = BN_new();
    gx = BN_new();
...
int32_t ec_make_public_from_components(key_spec *spec,
    ...
    cctx = EVP_PKEY_CTX_new_from_pkey(get_global_jostle_fips_ossl_lib_ctx(),
                                      pkey, NULL);
