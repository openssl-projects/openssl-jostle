...
/*
 * A public value y failed OpenSSL's own DH public-key check (not in the
 * subgroup generated by q, or otherwise structurally invalid). Reached from
 * dh_make_public_from_components (a q-bearing import) and, for a peer key
 * that bypassed the spec path (decoded straight from an
 * X509EncodedKeySpec SPKI), from kex_set_peer's peer_pubkey_check_failed
 * classifier. The OpenSSL reason is one of DH_R_CHECK_PUBKEY_INVALID,
 * DH_R_INVALID_PUBKEY, DH_R_CHECK_PUBKEY_TOO_SMALL or
 * DH_R_CHECK_PUBKEY_TOO_LARGE — one code serves all four since the remedy
 * does not differ.
 *
