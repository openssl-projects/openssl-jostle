...
/*
 * A public value y failed a DH public-key check: either OpenSSL's own
 * EVP_PKEY_public_check (not in range, or otherwise structurally invalid —
 * the OpenSSL reason is one of DH_R_CHECK_PUBKEY_INVALID,
 * DH_R_INVALID_PUBKEY, DH_R_CHECK_PUBKEY_TOO_SMALL or
 * DH_R_CHECK_PUBKEY_TOO_LARGE, one code serving all four since the remedy
 * does not differ), or dh_validate_public_key_subgroup's own follow-up test
 * that y actually lies in the subgroup q generates — a distinct constraint
 * EVP_PKEY_public_check does not enforce on a named safe-prime group.
 *
 * Reached from dh_make_public_from_components (a q-bearing components
 * import), from the generic SPKI decoder for a DHX (X9.42) key (decoded
 * straight from an X509EncodedKeySpec, which never reaches the components
 * path), and, for a peer key that skipped both of those, from
 * kex_set_peer's peer_pubkey_check_failed classifier.
 *
 * BouncyCastle's own KeyFactory refuses the same input at generatePublic
 * time, with java.security.spec.InvalidKeySpecException ("Y value does not
 * appear to be in correct group"). We refuse at the same site (KeyFactory
 * import) when q is supplied, and at doPhase for a peer that skipped it.
 */
#define JO_DH_PEER_PUBKEY_INVALID -180
