...
int32_t mlkem_generate_key_pair(key_spec *spec, int32_t type, uint8_t *seed, size_t seed_len, void *rnd_src) {
    ...
    switch (type) {
        case KS_ML_KEM_512:
            ctx = EVP_PKEY_CTX_new_from_name(get_global_jostle_fips_ossl_lib_ctx(), "ML-KEM-512",NULL);
            break;
        case KS_ML_KEM_768:
            ctx = EVP_PKEY_CTX_new_from_name(get_global_jostle_fips_ossl_lib_ctx(), "ML-KEM-768",NULL);
            break;
        case KS_ML_KEM_1024:
            ctx = EVP_PKEY_CTX_new_from_name(get_global_jostle_fips_ossl_lib_ctx(), "ML-KEM-1024",NULL);
            break;
        default:
