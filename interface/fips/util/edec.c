...
int32_t edec_generate_key(key_spec *spec, int32_t type, void *rnd_src) {
    ...
    switch (type) {
        case KS_ED25519:
            ctx = EVP_PKEY_CTX_new_from_name(get_global_jostle_fips_ossl_lib_ctx(), "ED25519",NULL);
            break;
        case KS_ED448:
            ctx = EVP_PKEY_CTX_new_from_name(get_global_jostle_fips_ossl_lib_ctx(), "ED448",NULL);
            break;
        default:
...
int32_t edec_decode_private_key(key_spec *key_spec, int32_t typeId, uint8_t *src, size_t src_len) {
    ...
    key_spec->key = EVP_PKEY_new_raw_private_key_ex(
        get_global_jostle_fips_ossl_lib_ctx(), type,NULL, src, src_len);
