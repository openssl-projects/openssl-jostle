...
/**
 * Input validation at the X.509 NI surface of the FIPS interface library
 * ({@link FIPSNISelector#X509NI}), mirroring {@code X509LimitTest}.
 * <p>
 * The FIPS glue is the base bridge re-included under renamed symbols and the
 * util twin differs only by the lib ctx accessor, so the checks are identical
 * by construction — this pins that they survived into the other library with
 * the same codes and the same messages, which is the only thing that catches a
 * wrapper that dropped an entry point or a tree that drifted. The handle-kind
 * tag in particular is a util-layer structure, so a divergence there would be
 * invisible to every functional test.
 * <p>
 * Driven directly so the bridge and util checks are reached rather than the
 * CertificateFactory SPI's.
 * <p>
...
{
    private final X509NI ni = FIPSNISelector.X509NI;
    ...
    public static void beforeAll()
    {
        ...
        // Constructing the PROVIDER is what runs the Loader; a static read of
        // FIPSNISelector alone would leave no native library loaded.
