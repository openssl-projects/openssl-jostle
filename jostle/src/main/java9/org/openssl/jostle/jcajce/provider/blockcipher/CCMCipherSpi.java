...
/**
 * Java 9+ override of {@link CCMCipherSpi}. Functionally identical to the
 * Java 8 baseline, but keeps the native context reachable across native
 * calls with {@link Reference#reachabilityFence(Object)} (a no-op JIT
 * hint) rather than {@code synchronized(this)}. The public/protected API
 * surface MUST stay identical to the Java 8 copy — only the
 * reachability mechanism differs.
 *
 * <p>See the Java 8 {@code CCMCipherSpi} for the full design notes on
 * CCM's one-shot constraint and the AAD discipline.
 */
public class CCMCipherSpi extends CipherSpi
{
            ...
            throws BadPaddingException
    {
        ...
        // reachabilityFence(this) keeps the native ctx reachable across
        // the call (see engineInit). The Java 8 baseline uses
