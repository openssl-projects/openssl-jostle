...
public class EdDSAKeyPairGenerator extends KeyPairGenerator
{
    ...
    public void initialize(AlgorithmParameterSpec params, SecureRandom random) throws InvalidAlgorithmParameterException
    {
        ...
        // MT-59. NamedParameterSpec is the JCA-idiomatic way to name an Edwards
        // variant and both references accept it; we refused it outright. Both
        // spec types are reduced to a NAME here and then run through the
        ...
        {
            askedName = ((EdDSAParameterSpec) params).getName();
        }
        else if (params instanceof java.security.spec.NamedParameterSpec)
        {
            askedName = ((java.security.spec.NamedParameterSpec) params).getName();
        }
        else
        {
            throw new InvalidAlgorithmParameterException(
                    "expected an EdDSAParameterSpec or a NamedParameterSpec, got "
                            + (params == null ? "null" : params.getClass().getName()));
        }

        // The map is keyed on the UPPERCASE spellings plus the EdDSAParameterSpec
        // constants, while NamedParameterSpec.getName() answers "Ed25519" in
        // mixed case. Try the raw name first so the existing EdDSAParameterSpec
        // path is byte-for-byte unchanged, then the uppercased form - which is
        // the case-insensitive match the design called for, added where it
        // cannot alter the pre-existing lookup.
        OSSLKeyType newType = paramToTypeMap.get(askedName);
        if (newType == null)
        {
            newType = paramToTypeMap.get(askedName.toUpperCase(java.util.Locale.ROOT));
        }

        if (newType == null)
        {
            throw new InvalidAlgorithmParameterException("unknown algorithm: " + askedName);
        }
