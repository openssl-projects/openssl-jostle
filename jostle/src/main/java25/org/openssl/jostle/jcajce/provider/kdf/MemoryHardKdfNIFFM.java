...
/**
 * FFM bridge for the memory-hard KDFs (scrypt, Argon2).
 *
 * <p>Separate from {@link KdfNIFFM} because the FIPS FFM class extends that one
 ...
 * {@link KdfNIFFM}: both KDFs run for a caller-controlled duration (high cost
 * parameters take seconds by design), which is the worst case for pinning the
 * caller's arrays.</p>
 */
public class MemoryHardKdfNIFFM implements MemoryHardKdfNI
{
    private static final Logger L = Logger.getLogger("MEMHARD_KDF_NI_FFM");
    private static final Linker linker = Linker.nativeLinker();

    private final MethodHandle scryptFuncHandle;
    private final MethodHandle argon2FuncHandle;

    public MemoryHardKdfNIFFM()
    {
        this(SymbolLookup.loaderLookup());
    }

    public MemoryHardKdfNIFFM(SymbolLookup lookup)
    {
    ...
    public int scrypt(byte[] password, byte[] salt, int n, int r, int p, byte[] out, int outOffset, int outLen)
    {
        ...
        {
            L.log(Level.WARNING, "FFM JoKDF_SCRYPT", t);
                      ...
                      int lanes, byte[] out, int outOffset, int outLen)
    {
        ...
        {
            L.log(Level.WARNING, "FFM JoKDF_ARGON2", t);
            throw new RuntimeException(t.getMessage(), t);
        }
    }
