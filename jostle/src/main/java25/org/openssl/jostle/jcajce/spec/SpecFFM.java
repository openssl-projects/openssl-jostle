...
public class SpecFFM implements SpecNI
{
    ...
    public SpecFFM(SymbolLookup lookup)
    {
        this(lookup, "");
    }
     ...
     */
    public SpecFFM(SymbolLookup lookup, String symPrefix)
    {
    ...
    public int ni_encap(long keyRef, String opt, byte[] secret, int inOff, int inLen, byte[] out, int off, int len, RandSource randSource)
    {
        ...
        // segments would let a non-overlapping alias half-succeed on FFM while
        ...
        try (Arena a = Arena.ofConfined())
        {
            ...
            // unwritten arena bytes — the exact JNI/FFM divergence this guards
