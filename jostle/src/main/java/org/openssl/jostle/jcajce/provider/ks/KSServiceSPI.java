    ...
    extends KeyStoreSpi
{
        ...
        throws CertificateEncodingException
    {
        ...
        List<byte[]> encodedCertificates = new ArrayList<byte[]>(chain.length);
        int totalLength = 4;
        for (Certificate certificate : chain)
        {
            ...
            encodedCertificates.add(encoded);
            if (encoded.length > Integer.MAX_VALUE - totalLength - 4)
            {
                throw new CertificateEncodingException(
                        "certificate chain encoding is too large");
            }
            totalLength += 4 + encoded.length;
        }

        byte[] out = new byte[totalLength];
        int offset = writeInt(out, 0, encodedCertificates.size());
        for (byte[] encoded : encodedCertificates)
        {
            offset = writeInt(out, offset, encoded.length);
        ...
        throws CertificateException, IOException
    {
        ...
        if (encoded.length < 4)
        {
            throw new IOException("certificate chain encoding is truncated");
        }
        ...
        if (count < 0)
        {
            throw new IOException("certificate chain count is negative");
        }
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            {
                throw new IOException("certificate chain length is truncated");
            }
            int length = readInt(encoded, offset);
            offset += 4;
            if (length < 0 || length > encoded.length - offset)
            {
                throw new IOException("certificate chain certificate is truncated");
            }
            chain[i] = factory.generateCertificate(
                    new ByteArrayInputStream(encoded, offset, length));
            offset += length;
        }
        if (offset != encoded.length)
        {
            throw new IOException("certificate chain has trailing data");
        }
        return chain;
    }
