...
public interface DHServiceNI extends DefaultServiceNI
{
    ...
    /** Shared by the NI error arm and {@code DHKeyAgreementSpi}, same reason. */
    String PEER_PUBKEY_INVALID_MESSAGE = "peer public value failed the public-key check";
    ...
    default long handleErrors(long code)
    {
        ...
        switch (errorCode)
        {
            ...
            case JO_DH_PEER_PUBKEY_INVALID:
                // The peer's y failed OpenSSL's own DH public-key check (not
                // in the subgroup generated by q, or otherwise structurally
                // invalid). DHKeyAgreementSpi turns this into the
                // caller-facing InvalidKeyException.
                throw new IllegalArgumentException(PEER_PUBKEY_INVALID_MESSAGE);
