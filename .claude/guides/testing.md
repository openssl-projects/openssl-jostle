...
Cover, per entry point: `in == out` at the same offset; `in == out` with the output offset **below** the input offset (forces a backward overlap); and `in == out` with the output offset **above** the input offset (forward overlap). Run at NI and JCE level. If the probe shows a layout the bridge cannot do correctly, the fix is in the C (use `memmove`, or reject the aliased call explicitly) — not a test that documents the corruption.
