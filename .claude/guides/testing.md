...
2. **Break the copy the test JVM actually LOADS, and confirm the sabotage landed.** A green result after a sabotage is worthless unless you can show the sabotage was in effect — and two things routinely make it not be. **Multi-release:** for a class with `javaN/` overrides, a JDK-25 run serves the `java9/` (or later) copy, so editing `src/main/java/.../X.java` changes nothing the test sees. This has bitten twice in consecutive work items — WI-10's `getFormat()` sabotage and MT-5's KTS digest sabotage both "passed" against untouched loaded code. **Silent no-op edits:** a `sed` idiom that does not apply (BSD `sed` has no `0,/re/`), a pattern that no longer matches after an earlier edit. So: apply the sabotage, then `grep` the file you believe you changed and confirm the text is what you think, before reading any test result. Treat an unexpectedly-green falsification as "my sabotage failed" until proven otherwise, never as "the property holds anyway".
3. **Keep a control.** An untouched sibling test MUST stay green while the target tests fail. Worked example: sabotaging `mac_copy` failed `CloneStateTest.macCloneContinuesSameState`, `macClonesAreIndependentAndDiverge` and `macCloneAtStateBoundaries`, while `digestCloneContinuesSameState` and `digestClonesAreIndependentAndDiverge` stayed green. Without the control a broad failure (build broken, provider unregistered) is indistinguishable from the assertion firing.
...
The negative shifted-window check is the part that catches an off-by-one in the bridge.

**A worked instance of this rule being ignored, and what it cost.** The same 1-in-256 arithmetic applies to any assertion comparing a plaintext against a `copyOf` PREFIX of its ciphertext, and three such sites existed. Two drew a message length that could be 1 — `FIPSAESTest` at `1 + nextInt(1024)` and `FIPSDESedeAgreementTest` at `1 + nextInt(120)` — so at a 1-byte draw each compared a SINGLE ciphertext byte against a single plaintext byte. The AES one fired during a Tier 2 gate on `unitTest8` and cost a full gate plus a 20-run discriminator to classify, because an unseeded `SecureRandom` whose seed is never logged makes it non-reproducible by construction. The DESede one had roughly EIGHT times the rate and had simply not been unlucky yet — the likeliest flake in the matrix, found by censusing the shape rather than by failing. Both now guard the comparison on `length >= block size`; the third site (`FIPSNativeLengthSourcingTest`, fixed `byte[64]`) is sound as written and carries a comment saying so, because the obvious tidy-up is to make all three look alike.

Two rules follow. **A differentiator must not be deleted merely because it is flaky** — repair it. The DESede sentinel sits next to a SELF-roundtrip, which a uniformly-wrong implementation satisfies, so it was the only check in that branch proving encryption transforms its input; deleting it would have removed real coverage to fix a statistics bug. And **when one instance of a shape fires, census the shape** — the failing site is rarely the worst one. Without it, a bridge that wrote at `outOff-1` instead of `outOff` would still produce a buffer where the first `writtenLen` bytes happen to look like valid output (because they ARE valid output, just shifted). The shifted-window check forces the test to actually verify the boundary.
...
**Symptom this replaced: a JSLFIPS operation succeeded on a JSL public key, and did the work in mainline.** The two providers' asymmetric keys share Java classes (`JORSAPublicKey`, `JOECPrivateKey`, …), so a foreign key object is type-compatible everywhere. Until MT-14 the public half was accepted on that basis — public material carries no secret. That reasoning was about the KEY and the question is about the OPERATION: measurement (`fips-c-review/probes/xprovider_key_probe.c`) shows OpenSSL binds a key to its creating provider for life and serves operations on it THERE, whatever lib ctx drove the call. So the accepted object silently moved the work outside the module, and no functional test could see it, because mainline and the module compute the same bytes.

1. **Both halves are isolated, both directions.** A key created by one provider instance, into any operation of another, is rejected typed at the SPI boundary: `InvalidKeyException` with `"<private|public> key was created by a different Jostle provider instance; encode it with getEncoded() and decode it through this provider's KeyFactory"`. `KeyGenerator` surfaces (ML-KEM, the TLS hybrids) throw `InvalidAlgorithmParameterException` instead — `engineInit`'s contract permits nothing else — and the hybrids' message names generating the pair instead of encoding it, because those keys have no encoding.
2. **Identity is the Provider REFERENCE, not the name and not the library.** `PKEYKeySpec.usableBy(Provider)` compares by `==`. `removeProvider` + `addProvider` swaps which instance a name resolves to, so a key can outlive the name that identified its maker; and `getInstance(alg, Provider)` never required registration in the first place.
3. **Unbound × unbound accepts, deliberately.** A spec built outside any provider (direct SPI construction — `new MLKEMKeyPairGenerator("ML-KEM-768")`) records `null`, and two such keys work together. Refusing would break every direct-SPI consumer. `KeySpecBindingTest` pins all four cells, and builds its unbound side by direct construction precisely because `KeyPairGenerator.getInstance(alg, "JSL")` no longer yields one.
4. **The library-level check (`getSpecNI() != keyFactory.ownSpecNI()`) stays on private keys and is NOT redundant.** Instance-equal implies library-equal, so it adds nothing for provider-mediated calls — but in the unbound realm the instance check passes on `null == null` and only the library check catches a private key crossing between two hand-wired NI backends.
5. **Encodings are the only crossing.** `getEncoded()` → decode through the target provider's KeyFactory, for the public half as well as the private one; `FIPSTestUtil.crossPublic` / `crossPrivate` are the test-side helpers. This is what a real peer does anyway — a TLS or CMS counterparty holds an SPKI, not a key object. The TLS hybrid KEMs have no encoding, so they have no crossing at all: `FIPSMLXKEMAgreementTest` interoperates through the raw share instead.

6. **A key `Cipher.unwrap` returns is bound to the unwrapping provider** (MT-10). It has to be, or `unwrap` hands back an object the same provider refuses on first use — which is what it did until then, because `engineUnwrap` resolved its KeyFactory through JCA order. Secret keys stay unbound `SecretKeySpec`s. `UnwrappedKeyBindingTest` / `FIPSUnwrappedKeyBindingTest` pin it; see the `engineUnwrap` rule in java-spi.md.

7. **To discriminate an INSTANCE pin from a NAME pin, make one instance selectively incapable** (MT-16). Two instances of the same provider class compute identical bytes, so no output distinguishes "the work happened in the instance I asked for" from "it happened in whatever the name resolves to". `StrippedJostleProvider` (test/provider/) removes one service from a fresh `JostleProvider`; drive the operation through THAT instance while a full one is installed under the name. A name pin borrows the capability and succeeds; an instance pin fails loudly. Two traps, both hit on the first attempt:
   1. **`Provider.removeService` does nothing here, and asking first makes it worse.** `JostleProvider.getService` is final, resolves out of the legacy `"<type>.<ALG>"` property entries, and memoises each `Service` it builds. So the removal must delete the PROPERTY, on a provider nobody has queried yet — calling `getService` to check what you are about to remove memoises it past the removal. The first version did exactly that and every test built on it passed with nothing stripped.
   2. **Remove the PRIMARY, not the alias.** `SHA-256` is an alias of `SHA2-256`; deleting the alias leaves the digest reachable. Resolve `Alg.Alias.<type>.<ALG>` first, as `getService` does. A vacuity guard that throws when the key is absent is what caught this — keep it.

   `JostleFIPSProvider` is `final`, so the FIPS twin has no such instance and discriminates on key binding plus module residency instead. The whole technique is only needed where the observable is *which provider did the work*; where a key comes back, `getProviderInstance()` answers directly.

`FIPSKeyIsolationTest` owns the contract (`rsaKeysAreIsolatedInBothHalves`, `keyIsolationCompleteAcrossAllAsymmetricFamiliesBothDirections`) — new asymmetric families must be added to its sweep, with the rejection for each half (exact message per "Pin the exception message" above) AND the re-decode positive, since a refuse-everything SPI passes the rejections alone. Surfaces that consume keys indirectly (a CertificateFactory's pinned `verify`, a KeyAgreement `doPhase`, a KEM KeyGenerator's encapsulate arm) pin their own view — `FIPSX509CertificateFactoryTest.jslGeneratedKeypair_functionsThroughFipsFactory` is the reference. Don't confuse this class of test with *wrong-algorithm* rejection (`"expected a DSAPublicKey from the Jostle provider"` for an RSA key into a DSA Signature): same exception type, different check, both worth pinning separately.

**Testing the flip itself.** `FIPSCrossInstanceKeyTest` asks OpenSSL which provider serves a key (`SpecNI.getKeyProvider`) and sweeps every family both providers share; it self-arms off whether specs are bound, so a PARTIAL flip arms it and is reported by name. Its blind spot, worth knowing: it probes binding on JSL-generated keys only, so a family left unbound on the FIPS side alone does not arm it — that one surfaced instead as a FIPS key refused by its own FIPS KeyAgreement (`ProvFIPSXDH` still calling the 4-arg `XECKeyPairGenerator`). When flipping a registration, audit every `new <Producer>(…)` in **both** `Prov*.java` trees for an argument that mentions `provider`.
