---
name: regen-fips-ffm-wrappers
description: Regenerate or verify the interface/fips/ffm/<x>_fips_ffm.c rename wrappers that give the FIPS interface library JoFIPS_-prefixed exports. Use whenever an FFM entry point is added, renamed or removed in either native tree, whenever FIPS_FFM_GLUE_SOURCES is edited, and as a check before declaring native work done — including phrases like "I added an FFM entry point", "regenerate the FIPS wrappers", "are the FIPS FFM symbols still disjoint", "why does the FIPS library not export X". Exists because a hand-edited or forgotten wrapper produces a FIPS library that silently lacks an entry point, or one whose twin is compiled twice.
---

# Regenerate the FIPS FFM rename wrappers
...
1. **An FFM entry point was added, renamed or removed** in either tree. The
   wrapper for that file needs a new/changed/dropped `#define`.
2. **A new `ffm/*.c` glue file was added.** It needs a whole new wrapper, and
   that wrapper needs a `FIPS_FFM_GLUE_SOURCES` entry.
3. **`FIPS_FFM_GLUE_SOURCES` was edited** — the checker catches a twin left
...
| wrapper not in `FIPS_FFM_GLUE_SOURCES` | the whole file is absent from the FIPS library |
...
another glue function across files**. If `md_ffm.c` ever called
`JoMAC_allocate`, that call sits in `md_ffm.c`'s translation unit, which carries
`md_fips_ffm.c`'s `#define` block and *not* `mac_fips_ffm.c`'s — so it would
...
1. **`openssl_fips_ffm.c`** — FIPS-only, already `JoFIPS_`-named, not a twin. It
   also owns `JoFIPS_get_openssl_errors`. There is **no `openssl_ffm.c` in the
...
2. **`rand_upcall_ffm.c`** — defines no `Jo` export (internal, called from C).
   Reported as "not wrapped" rather than treated as an error.
3. **`util/` symbols.** Only `ffm/` glue is wrapped. A util function the Java
   ...
   `ffm/ops_ffm.c` is the reference, twinned across both trees like any other
   glue file.
...
1. `FIPSLibraryLookupParityTest.everyResolvedFfmSymbolIsJoPrefixed` — every FFM
...
Background: the symbol-collision section of `.claude/guides/native-code.md`.
