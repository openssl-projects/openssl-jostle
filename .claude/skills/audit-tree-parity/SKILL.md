...
2. **Nonfips-only files**: the algorithm families the FIPS provider doesn't ship — `edec`, `ks` and the memory-hard KDFs (`kdf_memhard`) in util/jni/ffm — plus `jni/open_ssl_jni.c` and `jni/native_info_jni.c` (base init/diagnostic glue; the fips tree has its own `openssl_fips_jni.c`). **ML-DSA / ML-KEM / SLH-DSA are NOT on this list any more** — the 3.5.x module implements all three, so they are ordinary twins as of 2026-08-23.
3. **Fips-only files**: `util/rand/jostle_fips_ctx.*`, `util/capability_fips.*` (the module-version and implementing-provider probes; the SHARED fetch probe `util/capability.*` is an ordinary twin, since both providers gate registrations on it), and every `*_fips_jni.c` / `*_fips_*` rename-re-include wrapper.
